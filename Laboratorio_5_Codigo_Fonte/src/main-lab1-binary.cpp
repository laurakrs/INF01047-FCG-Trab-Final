...
// constroem triangulos para renderizacao - funcoes para criacao dos digitos
GLuint BuildTriangles0 (GLfloat translacaox);
GLuint BuildTriangles1 (GLfloat translacaox);
...
// TAREFA 3 - implementa contador binario de quatro digitos, o qual eh incrementado em uma unidade por segundo
// 1 eh azul, 0 eh vermelho
// Opcoes: um VAO unico ou VAOs independentes 
// utiliza glfwGetTime() - Sugestão de uso: "int seconds = (int)glfwGetTime();".
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(500, 500, "INF01047 - 217870 - Laura Keidann Rodrigues da Silva", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    // Construimos representacao de um triangulo para as funcoes BuildTriangles da tarefa 2
    // Precisamos de 4 objetos
    GLuint vertex_array_object_id_0[4];
    GLuint vertex_array_object_id_1[4];

    GLfloat translacaox = 0.0; 

    // loop para chamar as funcoes para construir os triangulos
    for (int i = 0; i < 4; ++i){
        vertex_array_object_id_0[i] = BuildTriangles0(translacaox);
        vertex_array_object_id_1[i] = BuildTriangles1(translacaox);

        // muda o valor da translacao de maneira regular, reduzindo x em 0.5
        translacaox -= 0.5; 
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // TAREFA 3
        // 1a opcao = armazenar os vertices de todos os digitos em um unico VAO, e utilizar os parametros count e indices da funcao glDrawElements para selecionar quais devem ser utilizados para renderizacao
        // 2a opcao = VAOs independentes

        // "Ligamos" os VAOs de acordo com a passagem de tempo
        // Pedimos para a GPU rasterizar os vertices apontados pelo respectivo VAO

        int seconds = (int)glfwGetTime(); // funcao para pegar o tempo

        // loop para ver qual numero desenhar
        for(int i = 0; i < 4; ++i){
            if(seconds % 2 == 0){ // se o numero de segundos eh par, eh 0 
                glBindVertexArray(vertex_array_object_id_0[i]);
                glDrawElements(GL_TRIANGLE_STRIP, 34, GL_UNSIGNED_BYTE, 0);
            }else{ // se o numero de segundos eh impar, eh 1
                glBindVertexArray(vertex_array_object_id_1[i]);
                glDrawElements(GL_TRIANGLES, 12, GL_UNSIGNED_BYTE, 0);
            }

            // divide seconds por 2 para pegar o proximo numero que deve ser desenhado
            seconds /= 2; 
...
// Constrói triângulos para futura renderização do 0 
GLuint BuildTriangles0(GLfloat translacaox)
{
    // Primeiro, definimos os atributos de cada vértice.

    // Horizontalmente: Raio externo 0.2 (NDC), raio interno 0.1 (NDC)
    GLfloat externalRadiusX = 0.2f;
    GLfloat internalRadiusX = 0.1f;

    // Verticalmente: Raio externo 0.5 (NDC), raio interno 0.4 (NDC)
    GLfloat externalRadiusY = 0.5f;
    GLfloat internalRadiusY = 0.4f;

    // Número de vértices externos e internos
    GLint numberOfVertices = 32;
    GLint numberOfSides = numberOfVertices - 1;
    ...
    // Vetores para armazenar as coordenadas dos vértices externos e internos
    GLfloat circleVerticesX[numberOfVertices];
    GLfloat circleVerticesY[numberOfVertices];
    GLfloat circleVerticesZ[numberOfVertices];

    // define as variaveis de partida
    GLfloat x = 0.75f + translacaox;
    GLfloat y = 0.0f;
    GLfloat z = 0.0f;
    ...
    // Loop para gerar as coordenadas dos vértices externos e internos
    for (int i = 0; i < numberOfVertices; ++i)
    {
        GLfloat angle = i*doublePi/numberOfSides;

        if(i%2 == 0){ // se for par, raio interno
            circleVerticesX[i] = x + (internalRadiusX * cos(angle));
            circleVerticesY[i] = y + (internalRadiusY * sin(angle));
            circleVerticesZ[i] = z;
        }else{ // se for impar, raio externo
            circleVerticesX[i] = x + (externalRadiusX * cos(angle));
            circleVerticesY[i] = y + (externalRadiusY * sin(angle));
            circleVerticesZ[i] = z;
        }
        
        // Armazena as coordenadas NDC (Normalized Device Coordinates)
        NDC_coefficients[i * 4] = circleVerticesX[i];
        NDC_coefficients[i * 4 + 1] = circleVerticesY[i];
        NDC_coefficients[i * 4 + 2] = circleVerticesZ[i];
        NDC_coefficients[i * 4 + 3] = 1.0f; // W = 1.0
    }
    ...
    for (int i = 0; i < numberOfVertices; i++)
    {
        // Os vertices do circulo sao vermelhos
        color_coefficients[i * 4] = 1.0f;   // R
        color_coefficients[(i * 4) + 1] = 0.0f; // G
        color_coefficients[(i * 4) + 2] = 0.0f; // B
        color_coefficients[(i * 4) + 3] = 1.0f; // Alpha 0.0 (fully transparent) -> 1.0 (fully opaque)
    }
...
// Constrói triângulos para futura renderização do 1
GLuint BuildTriangles1(GLfloat translacaox)
{
    // Primeiro, definimos os atributos de cada vértice.
    ...
    // coeficientes para definir a geometria do numero 1
    GLfloat NDC_coefficients[] = {
    //    X                    Y       Z     W
         0.70f + translacaox,  -0.5f, 0.0f, 1.0f,
         0.70f + translacaox,  0.35f, 0.0f, 1.0f,
         0.65f + translacaox,  0.3f,  0.0f, 1.0f,
         0.70f + translacaox,  0.5f,  0.0f, 1.0f,
         0.80f + translacaox,  0.5f,  0.0f, 1.0f,
         0.80f + translacaox, -0.5f,  0.0f, 1.0f,
    };
    ...
    // Os vertices do numero 1 sao azuis
    ...
    // desenharemos 4 triangulos formados pelos seguintes vertices
    // 0,1,4 = triangulo esquerdo (inferior)
    // 5,0,4 = triangulo direito (inferior)
    // 1,2,3 = pontinha do 1
    // 1,3,4 = triangulo superior
  

    GLubyte indices[] = { 0,1,4, 5,0,4, 1,2,3, 1,3,4 }; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
