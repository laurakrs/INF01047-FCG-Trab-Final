...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - 217870 - Laura Keidann Rodrigues da Silva", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec4 camera_movement;
        glm::vec4 camera_position_c  = glm::vec4(x_start,y_start,z_start,1.0f) + camera_movement; // + movement; // Ponto "c", centro da câmera
...
void CursorPosCallback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    // Tarefa 4: Ao pressionar o botão esquerdo do mouse e arrastar na tela, a câmera deve rotacionar na horizontal e na vertical 
    // (mudança do vetor "view" da câmera), sem alterar sua posição, e preservando o vetor "up" igual ao eixo Y positivo (global). 
    // Esta movimentação simula, por exemplo, rotações da cabeça de um personagem em primeira pessoa.
...
//deve somente ser utilizada para controlar o valor de variáveis que representam o estado das teclas (pressionadas ou não)
